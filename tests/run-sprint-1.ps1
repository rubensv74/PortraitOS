...
$runnerPath = Join-Path $testRoot "sprint-1-runner.html"
$temporaryRoot = Join-Path $env:TEMP ("portraitos-sprint-1-" + [guid]::NewGuid().ToString("N"))
...
$arguments = @(
    ...
    "--allow-file-access-from-files", "--user-data-dir=$profilePath",
    "--virtual-time-budget=120000", "--dump-dom", $runnerUrl
)
