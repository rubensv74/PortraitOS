...
# ============================================================
# PortraitOS Sprint 4 — Deterministic Storage Harness (HTTP)
# ------------------------------------------------------------
# Sirve el repositorio en un puerto libre con HttpListener y
# ejecuta tests/sprint-4-runner.html en Chrome headless.
# El runner publica sus resultados vía POST /results; este
# script espera en tiempo real el archivo de resultados (sin
# --virtual-time-budget, que es determinista pero no fiable
# con I/O real: IndexedDB, navigator.storage.estimate y
# FileReader). Limpia siempre (try/finally).
# ============================================================
...
$runnerName = "sprint-4-runner.html"
$temporaryRoot = Join-Path $env:TEMP ("portraitos-sprint-4-" + [guid]::NewGuid().ToString("N"))
...
# Servidor HTTP en un proceso PowerShell separado: GetContext bloqueante
# (fiable para servir) y terminación por taskkill /T (sin deadlock de Stop-Job).
